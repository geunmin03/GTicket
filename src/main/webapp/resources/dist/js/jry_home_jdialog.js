(function ($) {
	...
	$.fn.jAlert_OLD = function (options) {
        var panel = this;
        this.option = jQuery.extend({ msg: '', loc: '', yeslogin: '', modal: true, buttons: { '확인': function () { $(this).dialog('close'); } } }, options);
	...
	$.fn.jSlimPopup_OLD = function (options) {
        var panel = this;
        this.option = jQuery.extend({
            modal: false,
            width: 200,
        ...
        $('#dialogPopup').dialog({
 	        ...
 	        open: function () {
 	            ...
 	            // 내용을 불러 옵니다.
 	            //$(this).load(url);
 	            $(this).dialog().parents(".ui-dialog").find(".ui-dialog-titlebar").hide();
 	            $(this).parent().removeClass('ui-corner-all');
 	            $(this).dialog().parents(".ui-dialog").find(".ui-dialog-content").css("padding", "0 0");

 	            $(this).load(this.option.url, function (response, status, xhr) {
 	                if (status == "success") {
                       
 	                    //$('#dialog_close_link').click(function () {
 	                    //    $('#dialogPopup').parents(".ui-dialog").find(".ui-dialog-titlebar-close").trigger("click");
 	                    //    return false;
 	                    //});
