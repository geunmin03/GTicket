...
function jsf_def_InputOnlyNumberOkCharacterZipCode(event) {

    var keyValue = event.key;
    var keyCode = event.which;
    var check = /[^(a-zA-Z0-9)\s\-]/; //-, space 추가
    var isCheck = !check.test(keyValue);
    if (keyCode > 122) {
        isCheck = false;
    }
...
function jsf_com_AjaxLoader(jpType, jpTopMargin, jpLeftMargin, jpIndex, jpSelectorID) {
    var loaderImgObj = $("#imgAjaxLoader" + jpIndex);
    var thisObj = $(jpSelectorID);
...
function jsf_com_GoToAnchor(jpSelector) {
    $('body,html').animate({ scrollTop: $(jpSelector).offset().top }, 'slow');
}

function jsf_com_GoToAnchor_ScrollSpeed(jpSelector, scrollspeed) {
    $('body,html').animate({ scrollTop: $(jpSelector).offset().top }, scrollspeed);
}
...
function jsf_com_GoToTop() {
    ...
    $('body,html').animate({ scrollTop: 0 }, (jvMiliSeconds == '' ? 300 : jvMiliSeconds));
}
        ...
        if (joCtrl.length > 0) {
            joCtrl.each(function (i) { 
                if($(this).parent().length == 1) {
                    $(this).parent().unbind("selectstart.ie9noselect").bind("selectstart.ie9noselect", function (e) {
                        e.preventDefault();
                    });
                }
            });
...
$(document).ready(function () {// When DOM is loaded
    $("a, img, input:button, input:submit, button, area").bind("focus.siteall", function () { $(this).blur(); });
    $("a.download").bind("click.sitedownload", function () { if ($("#_downloadframe").length == 0) $("<iframe name='_downloadframe' id='_downloadframe' frameborder='0' width='0' height='0'></iframe>").appendTo('body'); $.download('/Pages/Func/Downloading.aspx', 'Path=' + $(this).attr("dpath") + '&Name=' + $(this).attr("dname")); });
    $("img[multimedia_photo]").bind("click.multimedia_photo", function () { $('#dialogPopup').jPopup({ 'modal': true, 'width': 200, 'height': 200, 'url': '/Pages/Popup/Multimedia/ShowImage.aspx?fileurl=' + $(this).attr("fileurl"), 'title': '사진 보기' }); });
    $("img[multimedia_movie]").bind("click.multimedia_movie", function () { $('#dialogPopup').jPopup({ 'modal': true, 'width': 200, 'height': 200, 'url': '/Pages/Popup/Multimedia/ShowMovie.aspx?fileurl=' + $(this).attr("fileurl"), 'title': '멀티미디어 보기' }); });
    //$(document).bind('contextmenu.siteall', function (e) { return false; });
    $("#imgAjaxLoader").css({ 'z-index': 1000 });
