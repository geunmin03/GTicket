...
function jsf_base_ShowPerfSaleProcess() {
    ...
    if (jgBookTPopup == jcMODE_JQUERY) {// JQuery MODE        
        $('#dialogPopup').jPopup({ 'modal': true, 'width': 972, 'height': 583, 'url': 'http://'+location.host + '/Pages/Perf/Sale/PerfSaleProcess.aspx' + paramString, 'title': '공연예매' });
    } else {// Window MODE
...
function jsf_base_GoPerfSalePerf(jsp_IdPerf, jsp_IsLogin, HcadOption) {    
    if (jsp_IsLogin == '0') {
        ...
        if (jgSiteAlert == jcMODE_JQUERY) {// JQuery MODE
            $('#dialogAlert').jAlert({ "msg": '티켓 예매는 로그인이 필요합니다.', 'loc': jvRetUrl });
        } else {// Window MODE
...
function jsf_base_ShowPackageSaleProcess() {
    ...
    if (jgBookPPopup == jcMODE_JQUERY) {// JQuery MODE
        $('#dialogPopup').jPopup({ 'modal': true, 'width': 927, 'height': 571, 'url': '/Pages/Perf/Sale/PackageSaleProcess.aspx' + paramString, 'title': '패키지예매' });
    } else {// Window MODE
...
function jsf_base_GoPackageSale(jsp_IdPackage, jsp_IsLogin) {
    if (jsp_IsLogin == '0') {
        ...
        if (jgSiteAlert == jcMODE_JQUERY) {// JQuery MODE
            $('#dialogAlert').jAlert({ "msg": '패키지 예매는 로그인이 필요합니다.', 'loc': jvRetUrl });
        } else {// Window MODE
...
function jsf_base_SearchGo() {
    if (document.getElementById('searchbox').value.length < 0) {
        $("#dialogAlert").jAlert({ "msg": "검색어를 입력하세요." });
        return false;
    }
    else if (document.getElementById('searchbox').value.length < 2) {
        $("#dialogAlert").jAlert({ "msg": "검색어를 두자이상 입력하세요." });
        return false;
    } else {
