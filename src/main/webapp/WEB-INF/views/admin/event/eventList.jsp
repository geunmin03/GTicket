	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				<h1>공연 리스트 <small>Event List</small></h1>
				<ol class="breadcrumb">
					<li><a href="#"><i class="fa fa-dashboard"></i> 공연 관리</a></li>
							...
							<div class="box-body">
								<table class="table table-striped text-center">
									...
									<c:if test="${empty eventList}">
										<tr>
											<td colspan="10"> 
												<p style="padding:50px 0px; text-align: center;">검색한 공연이 존재하지 않습니다.<td colspan="10"> 
									...
									<c:forEach items="${eventList}" var="event">
										<tr>
											...
											<td >${event.evt_num}</td>
											<td >
												<img src="/admin/event/displayFile?fileName=${event.evt_img}" style="width:80px; hight:40px;">
												

												<!--  용도?-->
												<%-- <input type="hidden" name="img_${event.evt_num}" value="${event.evt_img}" /> --%>



											</td>
											<td class="col-md-2"><a
												href="/admin/event/eventDetail${pm.makeSearch(pm.cri.page)}&evt_num=${event.evt_num}"
												style="color: black;"> ${event.evt_name} </a></td>
												<td>${event.evt_company}</td>
											<td class="col-md-1">${event.evt_price}</td>
											<td><input name="amount_${event.evt_num}" type="number" style="width:80px; height:34px; padding-left:5px;" min="1"  value="${event.evt_amount}" /></td>
											<td class="col-md-1">${event.evt_discount}</td>
											<td class="col-md-1" ><fmt:formatNumber  value="${event.evt_amount - event.evt_discount}" pattern="###,###,###" /></td>
											
											
											<!-- 상품 전시(보임/숨김)기능 -->
											<td>
												<select class="form-control" name="buy_${event.evt_num}" style="width: 60px; display: inline-block;">
												  <option <c:out value="${event.evt_buy == 'Y'?'selected':''}"/>>Y</option>
												  <option <c:out value="${event.evt_buy == 'N'?'selected':''}"/>>N</option>
												</select>
											</td>
											<td class="col-md-2">
												<form class="deleteForm" method="post"
													...
													<!-- 상품 코드 -->
													<input type="hidden" name="evt_num"
														value="${event.evt_num}">
													<!-- 파일 이미지명 -->
													<input type="hidden" name="evt_img"
														value="${event.evt_img}">
														<!-- 수정기능 -->
													<span>
													<button type="button" name="btn_edit" class="btn btn-default" onclick="clickEdit(${event.evt_num});">수정</button>
													
													<!-- 삭제기능 -->
													<button type="button" name="btn_delete" class="btn btn-danger">삭제</button>
													<button type="button" name="btn_buy" class="btn btn-danger" onclick="clickBuy(${event.evt_num});">예매</button>
													</span>
												</form>
											</td>
										</tr>
